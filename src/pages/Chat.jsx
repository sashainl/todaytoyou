...
4. **자연스러운 대화**: 각 메시지가 독립적이지 않고 대화의 흐름을 이어가세요.`)
  ...
  const quickEmotion = (emotion) => {
    // 이모지에 해당하는 임시 메시지 설정 (실제 사람이 작성한 것처럼 자연스러운 내용)
    const emotionMessages = {
      '😊': '오늘 하루가 정말 좋았어. 날씨도 좋고 기분도 좋아서 뭔가 모든 게 잘 풀리는 느낌이야',
      '😢': '오늘 하루가 좀 힘들었어. 뭔가 마음이 무겁고 슬픈 기분이 들어',
      '😰': '요즘 불안하고 걱정되는 일이 많아. 미래가 걱정되고 어떻게 해야 할지 모르겠어',
      '😡': '오늘 정말 화가 났어. 뭔가 억울하고 속상한 일이 있었는데 말로 표현하기 어려워',
      '😴': '오늘 하루 종일 피곤했어. 일도 많고 잠도 제대로 못 잔 것 같아서 몸도 마음도 지쳐',
      '🤔': '요즘 마음이 복잡해. 뭔가 결정해야 할 일이 있는데 어떻게 해야 할지 잘 모르겠어'
    }
    
    const message = emotionMessages[emotion] || emotion
    
    // 임시 메시지를 입력 필드에 저장 (사용자가 확인 후 전송 가능)
